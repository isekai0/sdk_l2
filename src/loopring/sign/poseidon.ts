...
function getPseudoRandom(seed: any, n: any) {
  const res = [];
...
export function getMatrix(t: any, seed: any, nRounds: any) {
...
export function getConstants(t: any, seed: any, nRounds: any) {
  if (typeof seed === 'undefined') seed = SEED;
  if (typeof nRounds === 'undefined') nRounds = NROUNDSF + NROUNDSP;
  if (typeof t === 'undefined') t = T;
...
function ark(state: any, c: any) {
  for (let j = 0; j < state.length; j++) {
    state[j] = F.add(state[j], c);
  }
}

function sigma(a: any) {
...
export function createHash(t: any, nRoundsF: any, nRoundsP: any, seed?: any) {
  if (typeof seed === 'undefined') seed = SEED;
